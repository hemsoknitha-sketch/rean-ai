...
class VIPManager:
    ...
    @classmethod
    def add_vip(cls, user_id: int, name: str = "VIP User", days: int = 30, is_lifetime: bool = False) -> str:
        """Grants or extends VIP access for a user."""
        cls._ensure_loaded()
        uid_str = str(user_id)
        now = datetime.now()
        ...
        if is_lifetime or days <= 0:
            ...
            # If user is already active VIP, extend from current expiry date
        ...
        cls.save()
        logger.info(f"Granted VIP to user {user_id} ({name}) until {expiry_display}.")
        return expiry_display

    @classmethod
    def revoke_vip(cls, user_id: int) -> bool:
        """Revokes VIP access for a user."""
        cls._ensure_loaded()
        uid_str = str(user_id)
    ...
    def list_all_vips(cls) -> List[Dict]:
        """Lists all registered VIP users with current subscription status."""
        cls._ensure_loaded()
        result = []
